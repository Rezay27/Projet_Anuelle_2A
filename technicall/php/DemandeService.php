        ...
        <?php while (($demandes = $demande->fetch())) { ?>
            ...
            <div id="popup" class="blocpopup">
                <form id="demandeservice" action="verifdemandeservice.php" name="demandeservice" method="post"
                    ...
                    <?php
                    $reponse = $bdd->query("SELECT * FROM membre inner join abonnements on membre.id_abonnement = abonnements.id_abonnement where id_membre='{$_SESSION['id']}' ");
                    $donnees = $reponse->fetch();
                    if(isset($donnees['id_abonnement'])){
                    if ($donnees['abonnement_ponctuel'] == 0 ) {
                        ?>
                        ...
                        <p> * Votre demande est directement enregistrer </p>
                    <?php } else {
                        ?>
                        <input class="submitservice" type="submit" name="payer" value="Payer"/>
                        <input class="submitservice" type="submit" name="fermer" value="Annuler"/>
                        <p> * Vous allez être redirigez vers la page de paiement </p>
                    <?php }
                    } else {?>
                        <input class="submitservice" type="submit" name="payer" value="Payer"/>
                        <input class="submitservice" type="submit" name="fermer" value="Annuler"/>
    ...
    <div class="blocpopup" id="popupadd">
        ...
        <form id="add_service" action="verifaddservice.php" name="addservice" method="post" enctype="multipart/form-data">
            ...
            if ($donnees['abonnement_ponctuel'] == 0) {
                ...
                <p> * Votre demande sera directement enregistrée une fois le service validé </p>
            <?php } else {
                ?>
                <input class="submitservice" type="submit" name="non_abo_valider" value="Envoyer"/>
                <input class="submitservice" type="submit" name="fermer" value="Annuler"/>

                <p> * Vous recevrez une notification de l'avancement de la validation de votre demande.  </p>
            <?php }
            } else {?>
                <input class="submitservice" type="submit" name="payer" value="Payer"/>
                <input class="submitservice" type="submit" name="fermer" value="Annuler"/>
