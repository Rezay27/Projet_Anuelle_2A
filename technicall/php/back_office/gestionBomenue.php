<section>
    <section class="bloc1BO">
      <section class="navBarBO">
        <a href ="gestionBOMembre.php">Gestion Membre </a>
        <a class="couleurBarre"> | </a>
        <a href ="gestionBOAdmin.php"> Gestion Administrateur</a>
          <a class="couleurBarre"> | </a>
          <a href ="gestionBOIntervenant.php"> Gestion Intervenant</a>
          <a class="couleurBarre"> | </a>
          <a href ="gestionBOHistorique.php"> Gestion Historique</a>
          <a class="couleurBarre"> | </a>
          <a href ="gestionBODemandes.php"> Gestion Demandes</a>
          <a class="couleurBarre"> | </a>
          <a href ="gestionBOService.php"> Gestion Services</a>
          <a class="couleurBarre"> | </a>
          <a href ="gestionBOAbonnement.php">Gestion Abonnement </a>
      </section>
    </section>
</section>
